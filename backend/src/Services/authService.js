...
const updatePlayerID = async (ID, playerID) => {
  try {
    const result = await pool.query(
      "UPDATE User SET playerID = ? WHERE ID = ?",
      [playerID, ID]
    );
    console.log("Affected rows:", result.affectedRows);
    if (result.affectedRows === 0) {
      console.error("No rows updated. Check if ID exists in the database.");
    }
    return playerID;
  } catch (error) {
...
const getCouple = async (ID) => {
  try {
    const [coupleID] = await pool.query(
      "SELECT coupleID FROM User WHERE ID = ?",
      [ID]
    );
    return coupleID;
  } catch (error) {
...
const updateCouple = async (ID, coupleID) => {
  try {
    await pool.query("UPDATE User SET coupleID = ? WHERE ID = ?", [
      coupleID,
      ID,
    ]);
    return coupleID;
...
const createCouple = async (ID1, ID2) => {
  try {
    const coupleID = uuid();
    await pool.query(
      "INSERT INTO Couple (coupleID, ID1, ID2) VALUES (UUID_TO_BIN(?, 1), ?, ?)",
      [coupleID, ID1, ID2]
    );
    return coupleID;
  } catch (error) {
...
const deleteDuplicateCouple = async (ID1, ID2) => {
  try {
    const [couples] = await pool.query(
      "SELECT * FROM Couple WHERE (ID1 = ? AND ID2 = ?) OR (ID1 = ? AND ID2 = ?)",
      [ID1, ID2, ID2, ID1]
    );
    if (couples.length === 2) {
      await pool.query(
        "DELETE FROM Couple WHERE (ID1 = ? AND ID2 = ?)",
        [ID1, ID2]
      );
      console.log(
        `Duplicate couples with IDs (${ID1}, ${ID2}) and (${ID2}, ${ID1}) deleted`
      );
    } else {
